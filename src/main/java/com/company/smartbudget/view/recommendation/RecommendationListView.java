...
public class RecommendationListView extends StandardListView<Recommendation> {
    ...
    @ViewComponent
    private CollectionContainer<Recommendation> recommendationsDc;

    @ViewComponent
    private InstanceContainer<Recommendation> recommendationDc;

    @ViewComponent
    private InstanceLoader<Recommendation> recommendationDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Recommendation> recommendationsDataGrid;
    ...
    public void onInit(final InitEvent event) {
        recommendationsDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("recommendationsDataGrid.createAction")
    public void onRecommendationsDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Recommendation entity = dataContext.create(Recommendation.class);
        recommendationDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("recommendationsDataGrid.editAction")
    public void onRecommendationsDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    public void onRecommendationsDcItemChange(final InstanceContainer.ItemChangeEvent<Recommendation> event) {
        prepareFormForValidation();

        Recommendation entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            recommendationDl.setEntityId(EntityValues.getId(entity));
            recommendationDl.load();
        } else {
            recommendationDl.setEntityId(null);
    ...
    private OperationResult saveEditedEntity() {
        Recommendation item = recommendationDc.getItem();
        ...
        dataContext.save();
        recommendationsDc.replaceItem(item);
        updateControls(false);
    ...
    private ValidationErrors validateView(Recommendation entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        recommendationsDataGrid.getActions().forEach(Action::refreshState);
